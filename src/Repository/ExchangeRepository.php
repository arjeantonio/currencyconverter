...
class ExchangeRepository extends ServiceEntityRepository
{
     ...
     */
    public function findBySourceCurrency($value): ?Exchange
    {
        return $this->createQueryBuilder('e')
            ->andWhere('e.source_currency = :val')
            ->setParameter('val', $value)
            ->orderBy('e.target_currency', 'ASC')
            ->getQuery()
            ->getOneOrNullResult();
    }

    /**
     * @return array Returns an array of currency codes
     */
    public function getCurrencyCodes(): array
    {
        $result =  $this->createQueryBuilder('e')
            ->select('e.source_currency')
            ->orderBy('e.source_currency', 'ASC')
            ->groupBy('e.source_currency')
            ->getQuery()
            ->getScalarResult();

        return array_column($result, 'source_currency');
    }
